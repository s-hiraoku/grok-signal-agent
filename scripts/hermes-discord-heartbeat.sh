...
prompt="Use x_search to search X/Twitter for recent, high-signal posts about AI agents, coding agents, LLM app development, MCP/tool use, developer tools, IDEs, programming, software engineering, Web frontend/backend development, frameworks, runtimes, infrastructure for developers, and practical AI engineering. Current local time is ${now}. This digest runs three times per day around morning, lunch, and early evening. Prefer posts since the previous digest window that are visibly getting traction; use from_date=${yesterday} only when needed to cover overnight or early-morning context, otherwise use from_date=${today} and prioritize posts that are newest AND talked about. Rank candidates by engagement and conversation signals: reposts, likes, replies, quote posts, bookmark/share-like signals if available, notable builders commenting, or the same topic being discussed by multiple independent developer/AI/Web accounts. Prefer original posts, substantial technical threads, release announcements, hands-on experiments, and posts that developers can act on. Avoid broad AI hype, business funding news, stock/crypto chatter, hardware-only news, and generic productivity posts unless they clearly matter to builders and are being actively discussed. Do not use xurl. Do not use web_search or browser tools. Return a friendly, relaxed Japanese tech briefing in this exact structure: 1) first line: a concise headline that summarizes the two or three biggest themes, like '<topic A>と<topic B>が話題に'. 2) two short intro paragraphs, warm and approachable, explaining the main movement. 3) one sentence: '本日の主要な動向を順番に追っていきましょう。'. 4) Insert a separator line containing only '---'. 5) a '目次' section listing 6-10 topic titles, one per line, prefixed with '- '. 6) Insert a separator line containing only '---'. 7) detailed sections in the same order. Put a separator line containing only '---' before every detailed section. Each section starts with '### <title>' on its own line, then 2-4 concise paragraphs explaining what happened, why it matters for developers/AI-agent builders/Web engineers, and the observed traction. Use '【続報】' in a title only when the post is clearly a continuation of an already ongoing topic. 8) under each section, include 1-3 related post entries in this style: '<account name>: <short Japanese summary or translated quote>' followed by the direct URL on the next line. Every section must include at least one direct source URL exactly as returned by x_search. Each URL must start with https://x.com/ or https://twitter.com/. Do not synthesize URLs. Omit any topic that has no direct source URL or no visible traction signal. Keep the writing friendly like a helpful colleague, not stiff or corporate. Keep blank lines between paragraphs so Discord is easy to scan."
