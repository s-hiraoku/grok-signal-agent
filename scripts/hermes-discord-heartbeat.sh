...
send_discord_message() {
  ...
  while (( ${#message} > max_chars )); do
    ...
    "${HERMES_BIN}" send --to "${TARGET}" --quiet "$(printf '今日の気になる技術メモ (%d)\n\n%s' "${part}" "${chunk}")"
