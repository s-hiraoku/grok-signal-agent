...
1. First line: a concise headline that summarizes the two or three biggest themes, like `<digest_prefix><topic A>と<topic B>が気になるよ`.
...
5. A `目次` section listing 8-12 topic titles, one per line, prefixed with `- `.
